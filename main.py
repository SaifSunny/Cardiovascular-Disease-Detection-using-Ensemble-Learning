...
age = st.slider('Your Age (Years)', min_value=0.0, max_value=120.0, value=60.0, step=0.2)
st.write(''' ''')
...
if st.button('Submit'):
    ...
    # Set the thresholds based on the average values
    height_threshold = average_height
    weight_threshold = average_weight

    # Filter unusual height and weight values
    data = data[(data['height'] >= height_threshold) & (data['weight'] >= weight_threshold)]
    ...
    rfclf = RandomForestClassifier(random_state=20)
